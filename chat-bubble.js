(function() {
  ...
  async function loadDependencies() {
    ...
    // Load React, ReactDOM and additional dependencies
    await loadScript('https://unpkg.com/react@18/umd/react.production.min.js');
    await loadScript('https://unpkg.com/react-dom@18/umd/react-dom.production.min.js');
    await loadScript('https://cdn.jsdelivr.net/npm/marked@4.0.0/marked.min.js');
    await loadScript('https://cdn.jsdelivr.net/npm/dompurify@3.0.1/dist/purify.min.js');
  ...
  function addStyles(shadowRoot) {
    ...
    style.textContent = `
      ...
      @media (min-width: 640px) {
        .notify-md\\:bottom-24 { bottom: 6rem; }
        .notify-md\\:right-6 { right: 1.5rem; }
        .notify-md\\:w-\\[400px\\] { width: 400px; }
        .notify-md\\:rounded-lg { border-radius: 0.5rem; }
        .notify-md\\:h-\\[80vh\\] { height: 80vh; }
      ...
      .notify-animate-pulse {
        animation: notify-pulse 2s infinite;
      }

      /* Enhanced screen size compatibility */
      @media (max-width: 640px) {
        .notify-chat-container {
          width: 100% !important;
  ...
  async function initApp() {
    ...
    const ChatBubble = () => {
      ...
      return React.createElement(React.Fragment, null, [
        React.createElement('div', {
          className: `notify-overlay ${isChatOpen ? 'notify-overlay-visible' : ''}`,
          onClick: () => setIsChatOpen(false),
          key: "overlay"
        }),

        React.createElement('button', {
          id: "notify-chat-button",
          className: `notify-fixed notify-bottom-6 notify-right-6 notify-bg-green-600 
                     notify-text-white notify-rounded-full notify-w-16 notify-h-16 
                     notify-flex notify-items-center notify-justify-center notify-shadow-lg 
                     notify-hover:bg-green-700 notify-transition-all notify-duration-300 
                     notify-hover:scale-105 notify-z-50 ${isChatOpen ? 'notify-animate-pulse' : ''}`,
          onClick: toggleChat,
        ...
        React.createElement('div', {
          ref: chatWindowRef,
          className: `notify-chat-container notify-fixed notify-bg-white 
          ...
          React.createElement(ScrollArea, { 
            ...
            React.createElement('div', { className: "notify-space-y-4" }, [
              messages.map((msg, index) => 
                React.createElement('div', {
                  className: `notify-message ${msg.sender === 'user' ? 'notify-user-message notify-flex notify-justify-end notify-items-end notify-gap-2' : 'notify-bot-message notify-flex notify-justify-start notify-items-end notify-gap-2'}`,
                  key: `message-${index}`
                }, [
                  ...
                  React.createElement('div', { 
                    className: `notify-max-w-[75%] ${msg.sender === 'user' ? 'notify-order-1 notify-bg-green-600 notify-text-white notify-rounded-2xl notify-rounded-br-none notify-p-4' : 'notify-order-2 notify-bg-gray-100 notify-text-gray-800 notify-rounded-2xl notify-rounded-bl-none notify-p-4'}`,
                    key: "message-bubble"
                  ...
                  msg.sender !== 'system' && React.createElement('div', { 
          ...
          React.createElement('div', { 
            ...
            React.createElement('div', { 
              className: "notify-bg-gray-100 notify-rounded-lg notify-p-2 notify-pl-4"
            }, [
              React.createElement('div', {
                role: "textbox",
                contentEditable: true,
                className: "notify-min-h-[40px] notify-max-h-[120px] notify-overflow-auto notify-rich-text notify-text-sm notify-text-gray-800 notify-focus:outline-none",
